...
- Первым делом нужно получить список с заказами, получаем его из исходного файла файла "format.json",  помощью модуля json вытаскиваем из него словарь(dict) со списком заказов
- ~~Десериализацуем~~ Переводим список заказов в виде словаря с набором таких же словарей в список с объектами
	...
	<img src="images/category_from_json.png" alt="класс категории из json" width=500>
	<img src="images/product_from_json.png" alt="класс товара из json" width=500>
	<img src="images/order_from_json.png" alt="класс заказа из json" width=500>
...
- Далее нам нужно из списка заказов выбрать только заказы в канун нового года (в декабре), я это реализовать при помощи фильтра (оставил только заказы, у которых месяц == 12)
	<img src="images/get_new_year_eve_orders.png" alt="фильтр новогодних заказов" width=500>
- Теперь нам нужно получить статистику категорий этих заказов (сколько раз заказывали товар каждой категории), как я это реализовал:
	...
	<img src="images/get_category_statistics.png" alt="получение статистики категорий" width=500>
...
- И наконец теперь мы можем получить из этой статистики самые популярные категории
	...
	<img src="images/get_most_popular_categories.png" alt="получение самых популярных категорий" width=500>
	...
	- Оставляю только категории с таких количеством заказов
...
<img src="images/result.jpg" alt="выполнение" width=500>
<img src="images/result_json.png" alt="result.json" width=500>

Если что-то изменить в исходной файле (поменял название одной категории):
<img src="images/result2.jpg" alt="выполнение2" width=500>
<img src="images/result_json2.png" alt="result.json2" width=500>

Поменял дату одного заказа(теперь он не в декабре):
<img src="images/result_data_changed.jpg" alt="выполнение3" width=500>
<img src="images/result_json3.png" alt="result.json3" width=500>
